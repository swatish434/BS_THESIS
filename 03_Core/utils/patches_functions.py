...
def create_patches(hsi_images, masks, patch_size=128, stride=None, fill_value_X=0):
    """
    Splits HSI images and corresponding masks into overlapping or non-overlapping 
    ...
    Parameters:
        ...
        stride (int, optional): Step size for patch extraction. If None, defaults to patch_size (no overlap).
    ...
    for hsi, mask in zip(hsi_images, masks):
        ...
        for i in range(0, h, stride):
            for j in range(0, w, stride):
                # Ensure we don't go out of bounds completely (though slicing handles it, we want consistent behavior)
                # Ideally, we allow the last patch even if it overlaps more or is padded.
                
                # Check if this start point + patch_size is way beyond image? 
                # No, just standard sliding window. 
                # If we want strict sliding window where we stop if we can't fit a full patch, that's different.
                # Here we stick to the existing padding logic for edges.
